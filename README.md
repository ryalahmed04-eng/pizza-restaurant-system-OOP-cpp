...
pizzashop.exe
Sample Flow
The shop displays its menu (10 pizzas with prices).
The user selects items and quantities to build an order.
The user enters customer details.
The order total is calculated, a 10% premium discount is applied, and reward points are earned.
The receipt is written to order_receipt.txt.
The order is processed from the queue and displayed with its total.
Project Structure
pizza-management-system/
├── main.cpp
└── README.md
